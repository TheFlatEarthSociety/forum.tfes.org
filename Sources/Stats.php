...
function DisplayStats()
{
	...
	// Worst lurkers
	$temp_lurk = cache_get_data('stats_worst_lurkers_members', 600);
	$members_result = $smcFunc['db_query']('', '
		SELECT id_member, real_name, posts, total_time_logged_in
		FROM {db_prefix}members ' . (!empty($temp_lurk) ? '
		WHERE id_member IN ({array_int:member_list_cached})' : 'WHERE posts > {int:no_posts}') . '
	...
	foreach ($context['worst_lurk'] as $i => $poster)
	{
		$context['worst_lurk'][$i]['lurk_percent'] = round(($context['worst_lurk'][0]['lurk_ratio'] * 100) / $poster['lurk_ratio']);
	}
	
	// Cache the ones we found for a bit, just so we don't have to look again.
	if ($temp_lurk !== $temp_lurk2)
		cache_put_data('stats_worst_lurkers_members', $temp_lurk2, 480);
