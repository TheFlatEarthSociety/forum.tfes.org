	...
	if (!$pre_query || !empty($topic_ids))
	{
		...
		while ($row = $smcFunc['db_fetch_assoc']($result))
		{
			...
			if (!empty($settings['message_index_preview']))
			{
				...
				$row['first_body'] = trim(strip_tags(preg_replace('/<div class=\"topslice_quote\">.*?<\/div>/', '', $row['first_body'])));
				...
				$row['last_body'] = trim(strip_tags(preg_replace('/<div class=\"topslice_quote\">.*?<\/div>/', '', $row['last_body'])));
