...
{
	global $smcFunc, $errorTests, $txt, $context, $salvageBoardID, $salvageCatID;
	...
	*/
	...
	$errorTests = array(
		...
		'missing_topics' => array(
			...
			'fix_processing' => function($row) use($smcFunc, $salvageBoardID) {
				...
				$smcFunc['db_query']('', "
					UPDATE {db_prefix}messages
					SET id_topic = {int:new_topic}, id_board = {int:id_board}
					WHERE id_topic = {int:id_topic}",
					array(
						'id_board' => $row['id_board'],
						'id_topic' => $row['id_topic'],
						'new_topic' => $newTopicID
					)
				);
			},
		...
		'polls_missing_topics' => array(
			...
			'fix_processing' => function($row) use($smcFunc, $salvageBoardID, $txt) {
				// Only if we don\'t have a reasonable idea of where to put it.
				if ($row['id_board'] == 0)
				{
					createSalvageArea();
			...
			'fix_processing' => function($row) use($smcFunc) {
				...
				$smcFunc['db_query']('', "
					...
					SET id_first_msg = {int:first_msg},
						id_member_started = {int:member_started}, id_last_msg = {int:last_msg},
						id_member_updated = {int:member_updated}, approved = {int:approved}
					WHERE id_topic = {int:topic_id}",
					array(
						'first_msg' => $row['myid_first_msg'],
						'member_started' => $memberStartedID,
						'last_msg' => $row['myid_last_msg'],
						'member_updated' => $memberUpdatedID,
		...
		'stats_topics2' => array(
			...
			'fix_processing' => function($row) use($smcFunc) {
				...
				$smcFunc['db_query']('', "
					...
					array(
						'replies' => $row['my_num_replies'],
						'topic' => $row['id_topic']
					)
				);
			},
			'message_function' => function($row) use(&$context, $txt) {
				// Just joking?
				if ($row['my_num_replies'] == $row['num_replies'])
					return false;
		...
		'stats_topics3' => array(
			...
			'fix_processing' => function($row) use($smcFunc) {
				...
				$smcFunc['db_query']('', "
					UPDATE {db_prefix}topics
					SET unapproved_posts = {int:unapproved}
					WHERE id_topic = {int:topic}",
					array(
						'unapproved' => $row['my_unapproved_posts'],
						'topic' => $row['id_topic']
					)
				);
			},
		...
		'missing_boards' => array(
			...
			'fix_query' => '
				...
				WHERE b.id_board IS NULL
					AND t.id_topic BETWEEN {STEP_LOW} AND {STEP_HIGH}
				GROUP BY t.id_board',
			'fix_processing' => function($row) use($smcFunc, $salvageCatID) {
				createSalvageArea();
				...
				$smcFunc['db_query']('', "
					UPDATE {db_prefix}topics
					SET id_board = {int:newboard}
					WHERE id_board = {int:oldboard}",
					array(
						'newboard' => $newBoardID,
						'oldboard' => $row['id_board']
					)
				);
				$smcFunc['db_query']('', "
					UPDATE {db_prefix}messages
		...
		'missing_categories' => array(
			...
			'fix_collect' => array(
				'index' => 'id_cat',
				'process' => function($cats) use($smcFunc, $salvageCatID) {
					createSalvageArea();
					$smcFunc['db_query']('', "
						UPDATE {db_prefix}boards
						SET id_cat = {int:catid}
		...
		'missing_posters' => array(
			...
			'fix_collect' => array(
				'index' => 'id_msg',
				'process' => function($msgs) use($smcFunc) {
					$smcFunc['db_query']('', "
						UPDATE {db_prefix}messages
						SET id_member = 0
		...
		'missing_parents' => array(
			...
			'fix_collect' => array(
				'index' => 'id_parent',
				'process' => function($parents) use($smcFunc, $salvageBoardID, $salvageCatID) {
					createSalvageArea();
					$smcFunc['db_query']('', "
						UPDATE {db_prefix}boards
						SET id_parent = {int:idparent}, id_cat = {int:idcat}, child_level = 1
						WHERE id_parent IN ({array_int:parents})",
						array(
							'parents' => $parents,
							'idparent' => $salvageBoardID,
							'idcat' => $salvageCatID
						)
					);
				},
		...
		'missing_polls' => array(
			...
			'fix_collect' => array(
				'index' => 'id_poll',
				'process' => function($polls) use($smcFunc) {
					$smcFunc['db_query']('', "
						UPDATE {db_prefix}topics
						SET id_poll = 0
		...
		'missing_calendar_topics' => array(
			...
			'fix_collect' => array(
				'index' => 'id_topic',
				'process' => function($events) use($smcFunc) {
					$smcFunc['db_query']('', '
		...
		'missing_senders' => array(
			...
			'fix_collect' => array(
				'index' => 'id_pm',
				'process' => function($guestMessages) use($smcFunc) {
					$smcFunc['db_query']('', "
		...
		'missing_cached_subject' => array(
			...
			'fix_full_processing' => function($result) use($smcFunc) {
				$inserts = array();
		...
		'missing_topic_for_cache' => array(
			...
			'fix_collect' => array(
				'index' => 'id_topic',
				'process' => function($deleteTopics) use($smcFunc) {
					$smcFunc['db_query']('', "
		...
		'report_missing_comments' => array(
			...
			'fix_collect' => array(
				'index' => 'id_report',
				'process' => function($reports) use($smcFunc) {
					$smcFunc['db_query']('', "
		...
		'group_request_missing_group' => array(
			...
			'fix_collect' => array(
				'index' => 'id_group',
				'process' => function($groups) use($smcFunc) {
					$smcFunc['db_query']('', '
						DELETE FROM {db_prefix}log_group_requests
