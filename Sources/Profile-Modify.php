...
{
	global $context, $profile_fields, $profile_vars, $sourcedir, $txt, $scripturl, $modSettings, $user_info, $old_profile, $smcFunc, $cur_profile, $language;
	...
	*/
	...
	$profile_fields = array(
		'aim' => array(
			...
			'input_validate' => function() use(&$value) {
				$value = strtr($value, ' ', '+');
				return true;
			},
		),
		...
		'karma_good' => array(
			...
			// Set karma_bad too!
			'input_validate' => function() use(&$value, &$profile_vars, &$cur_profile) {
				$value = (int) $value;
		...
		'smiley_set' => array(
			...
			'permission' => 'profile_extra',
			'preload' => function() use(&$context, $modSettings, $txt, $cur_profile) {
			...
			'input_validate' => function (&$value) use($modSettings) {
